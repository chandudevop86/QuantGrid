...
def _dhan_option_rows(symbol: str, strikes: list[int]) -> tuple[list[dict[str, Any]], str | None]:
    ...
    with _DHAN_OPTION_LOCK:
        ...
        try:
            ...
            logger.info("Dhan option chain received successfully")
...
def get_option_chain(
    ...
    step = max(1, int(step))
    try:
        access_role: str = str(_access.user.role) if hasattr(_access, "user") and _access.user is not None else (_role or "")
