...
export default function MarketChart({ support, resistance }: MarketChartProps) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await api.candles("NIFTY", "5m");
  ...
  const chart = useMemo(() => {
    ...
    if (Number.isFinite(support)) allPrices.push(Number(support));
    if (Number.isFinite(resistance)) allPrices.push(Number(resistance));
  ...
  return (
    <article className="qg-card qg-chart-card">
      <div className="qg-section-heading qg-chart-heading">
        <div><span>NIFTY · 5 minute</span><h2>Market chart</h2></div>
      ...
      {!loading && !error && chart && (
        <div className="qg-market-chart" role="img" aria-label="NIFTY candlestick chart with volume, VWAP, support, and resistance">
          <svg viewBox={`0 0 ${WIDTH} 310`} preserveAspectRatio="none">
            ...
            {Number.isFinite(resistance) && <line className="qg-level-line qg-resistance-line" x1="0" x2={WIDTH} y1={chart.priceY(Number(resistance))} y2={chart.priceY(Number(resistance))} />}
            {Number.isFinite(support) && <line className="qg-level-line qg-support-line" x1="0" x2={WIDTH} y1={chart.priceY(Number(support))} y2={chart.priceY(Number(support))} />}
