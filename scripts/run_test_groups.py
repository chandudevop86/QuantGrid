...
def main() -> None:
    ...
    for index, group in enumerate(_groups(files, args.groups), start=1):
        ...
        command = [sys.executable, "-m", "pytest", "-q", *[str(path.relative_to(ROOT)) for path in group]]
        if args.coverage:
            command.extend(["--cov=services/trading-service/Backend", "--cov-append", "--cov-report="])
        _run(command, timeout=args.group_timeout)

    if args.coverage:
        _run([sys.executable, "-m", "coverage", "report", "--fail-under", str(args.cov_fail_under)], timeout=60)
